...
suite('copilotPluginConverters', () => {
	...
	suite('toSdkHooks', () => {
		 ...
		 * cmd.exe without relying on shell-specific JSON escaping.
		 * The generated script path is quoted so it also works when the
		...
		function echoJsonCmd(value: object): { command: string; cleanup: () => void } {
			...
			// Quote the script path so the fixture also works when the checkout path
			// contains spaces (for example, the standard Windows workspace location).
