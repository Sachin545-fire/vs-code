...
export class ToggleHackerModeAction extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.toggleHackerMode',
			title: localize2('toggleHackerMode', 'Toggle Hacker Mode'),
			category: Categories.View,
			f1: true,
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Shift | KeyMod.Alt | KeyCode.KeyH,
				weight: KeybindingWeight.WorkbenchContrib
			}
		});
	}
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		targetWindow.document.body.classList.toggle('hacker-mode');
	}
}

registerAction2(ToggleHackerModeAction);

export class ToggleAutoZenAction extends Action2 {
	constructor() {
		super({
			id: 'workbench.action.toggleAutoZen',
			title: localize2('toggleAutoZen', 'Toggle Auto-Zen Mode (Zen Lime)'),
			category: Categories.View,
			f1: true,
			keybinding: {
				primary: KeyMod.CtrlCmd | KeyMod.Shift | KeyMod.Alt | KeyCode.KeyZ,
				weight: KeybindingWeight.WorkbenchContrib
			}
		});
	}
	...
	override async run(accessor: ServicesAccessor): Promise<void> {
		...
		const isHidden = layoutService.isVisible(Parts.SIDEBAR_PART);
		layoutService.setPartHidden(isHidden, Parts.SIDEBAR_PART);
	}
}
registerAction2(ToggleAutoZenAction);
