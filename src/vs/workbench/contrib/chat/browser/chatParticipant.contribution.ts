...
export class ChatCompatibilityNotifier extends Disposable implements IWorkbenchContribution {
	...
	private registerWelcomeView(chatExtension: IExtension) {
		...
		// Add NVIDIA NIM Welcome View
		this._register(viewsRegistry.registerViewWelcomeContent(ChatViewId, {
			content: localize('nvidiaNimWelcome', "You need to set up GitHub Copilot and be signed in to use Chat.\n\n### NVIDIA NIM\nYou can also configure the built-in [NVIDIA NIM](command:workbench.action.openSettings?%22nvidiaNim%22) provider to use Chat without Copilot."),
			when: ContextKeyExpr.has('github.copilot.hasByokModels'),
		}));
	}
}
