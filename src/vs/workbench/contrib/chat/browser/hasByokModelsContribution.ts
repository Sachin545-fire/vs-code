...
export class HasByokModelsContribution extends Disposable implements IWorkbenchContribution {
	...
	private _update(): void {
		...
		// Also check for programmatically registered non-Copilot vendors
		// (e.g. built-in providers like NVIDIA NIM)
		const hasProgrammaticByokVendor = this._languageModelsService.getVendors().some(v => v.vendor !== COPILOT_VENDOR_ID);
		if (hasProgrammaticByokVendor) {
			this._setResult(true);
			return;
		}
